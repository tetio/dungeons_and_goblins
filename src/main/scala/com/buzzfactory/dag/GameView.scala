...
object GameView {
  ...
  def update(floor: DungeonFloor, state: GameState): Unit = {
    ...
    @tailrec def _update(state: GameState): Unit = {
      ...
      println(s"View position: (${state.view.position.x}, ${state.view.position.y})(${state.view.size.width}, ${state.view.size.height})")
      ...
      readKey(true) match {
        ...
        case "Q" => end()
        case "UP" => update(DungeonFloor(floor), GameState(state, Position(0, -1)))
        case "DOWN" => update(DungeonFloor(floor), GameState(state, Position(0, 1)))
        case "LEFT" => update(DungeonFloor(floor), GameState(state, Position(-1, 0)))
        case "RIGHT" => update(DungeonFloor(floor), GameState(state, Position(1, 0)))
